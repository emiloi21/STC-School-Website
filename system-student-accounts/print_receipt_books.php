...
$stud_query = $conn->query("SELECT * FROM tbl_book_students WHERE reg_id='$trans_row[reg_id]'") or die(mysql_error());
...
<tr>
<td style="font-weight: lighter; width: 50%;">
<?php if($_GET['report_type']==='AR'){ ?>
AR #: 
<?php }else{ ?>
OR #: 
<?php } ?>
<strong style="font-size: 14px;"><?php echo $_GET['receipt_num']; ?></strong></td>
...
$tbl_book_payables_query = $conn->query("SELECT payment_term FROM tbl_book_payables WHERE payable_id='$particulars_row[category_id]'") or die(mysql_error());
$tbl_book_payables_row = $tbl_book_payables_query->fetch();
